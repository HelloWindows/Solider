        ...
        namespace FSMState {
            public class SwordmanAttack1 : IFSMState {
                ...
                public void DoBeforeEntering() {
                    ...
                    character.avatar.PlayQueued(new string[] { "attack_1", "attack_2" });
                } // end DoBeforeEntering

                public void Reason(float deltaTime) {
                    if (character.avatar.IsPlaying("attack_1")) return;
                    // end if
                    if (character.avatar.IsPlaying("attack_2")) return;
                    // end if
                    if (character.avatar.IsPlaying("attack_3")) return;
                    // end if
                    if (mode != AttackMode.NEGATE) {
                        switch (mode) {
                            ...
                            case AttackMode.DEFAULT:
                                character.avatar.Play("attack_3");
                                break;
                ...
                public void Act(float deltaTime) {
                    if (character.avatar.IsPlaying("attack_2")) {
